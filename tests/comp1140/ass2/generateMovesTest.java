...
public class generateMovesTest {
    ...
    /**
     * Tests the generatePurMoves method in PurCublino
     */
    @Test
    public void testGenerateMovesPur() {
        for (int i = 0; i < FULL_PUR_GAME_WITH_MOVES_STATES.length; i++){
            Boards board = new Boards(FULL_PUR_GAME_WITH_MOVES_STATES[i]);
            PurCublino pur = new PurCublino(Character.isUpperCase(FULL_PUR_GAME_WITH_MOVES_STATES[i].charAt(0)), board);

            List<String> moves = Arrays.stream(pur.generatePurMoves()).map(x -> x.getEncodedMove()).collect(Collectors.toList());
            HashSet generatedMove = new HashSet(moves);
            HashSet givenMoves = new HashSet(Arrays.asList(FULL_GAME_WITH_MOVES_MOVES[i]));
            assertEquals(generatedMove, givenMoves, "The moves generated by the method are "+ FULL_GAME_WITH_MOVES_MOVES[i] + ", which are valid.");

        }
    }

    /**
     * Tests the generateLegalMoves method in ContraCublino
     */
    @Test
    public void testGenerateMovesContra(){
        for (int i = 0; i < ContraStates.length; i++){
            Boards board = new Boards(ContraStates[i]);
            ContraCublino pur = new ContraCublino(Character.isUpperCase(ContraStates[i].charAt(0)), board);

            List<String> moves = Arrays.stream(pur.generateLegalMoves()).map(x -> x.getEncodedMove()).collect(Collectors.toList());
            HashSet generatedMove = new HashSet(moves);
            HashSet givenMoves = new HashSet(Arrays.asList(ContraMoves[i]));
            assertEquals(generatedMove, givenMoves, "The moves generated by the method are "+ ContraMoves[i] + ", which are valid.");

        }
    }

    /**
     * valid game states for Contra Cublino
     */
    public static String[] ContraStates = {
    ...
    /**
     * Legal moves of each Contra Cublino state
     * In the order of the given game states
     */
    public static String[][] ContraMoves = {
